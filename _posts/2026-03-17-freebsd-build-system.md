...
Personally, I might lean towards using the worktree approach, since switching branches could add to the number of times where you have to face massive kernel rebuilds. This is especially the case for OS development, where a single overwrite by git [on branch switch] is likely happened to a core kernel file on the upper end of the import chain for thousands of indirectly dependent files. 
...
To change the build directory for a specific build, you can use the `MAKEOBJDIRPREFIX` variable when invoking `make`, for example:
```shell
  cd /usr/src
  env MAKEOBJDIRPREFIX=/usr/obj_mte_new make -j8 _worldtmp _legacy _bootstrap-tools
  env MAKEOBJDIRPREFIX=/usr/obj_mte_new make installincludes
```
...
## Useful `make` flags to speed up builds
...
- ``-DWITHOUT_CLEAN``: this option tells the build system to skip the cleaning step before building. It is particularly useful when there are little changes in the source code and to avoid time-consuming full rebuilds. Most useful in ``make buildkernel -DWITHOUT_CLEAN`` or ``make buildworld -DWITHOUT_CLEAN``.
